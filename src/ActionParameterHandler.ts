import { plainToClass } from "@nest/class-transformer";
import {
  validateOrReject as validate,
  ValidationError,
...
export class ActionParameterHandler<T extends BaseDriver> {
  ...
  handle(action: Action, param: ParamMetadata): Promise<any> | any {
    ...
    const value = this.normalizeParamValue(
   ...
   */
  protected async normalizeParamValue(
    value: any,
    param: ParamMetadata
  ): Promise<any> {
    ...
    if (
      typeof value === "object" &&
      ["queries", "headers", "params", "cookies"].some(
      ...
      await Promise.all(
        Object.keys(value).map(async (key) => {
          ...
          if (typeof keyValue === "string") {
            const ParamType: Function | undefined = Reflect.getMetadata(
              "design:type",
    ...
    if (
      ["number", "string", "boolean"].indexOf(param.targetName) === -1 &&
   ...
   */
  protected normalizeStringValue(
    value: string,
