...
export class RoutingControllers<T extends BaseDriver> {
  ...
  protected handleCallMethodResult(
    ...
    options: Action,
    interceptorFns: Array<Function>,
  ): any {
    if (isPromiseLike(result)) {
      return result
        .then((data: any) => this.handleCallMethodResult(data, action, options, interceptorFns))
        .catch((error: any) => this.driver.handleError(error, action, options));
    } else {
      if (interceptorFns) {
    ...
    return uses.map(use => {
      if (use.interceptor.prototype && use.interceptor.prototype.intercept) {
        ...
        return (action: Action, result: any) =>
          (getFromContainer(use.interceptor) as InterceptorInterface).intercept(action, result);
      }
