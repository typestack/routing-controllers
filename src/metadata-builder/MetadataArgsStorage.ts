...
export class MetadataArgsStorage {
    ...
    public filterControllerMetadatasForClasses(classes: Array<Function>): Array<ControllerMetadataArgs> {
        return this.controllers.filter(ctrl =>
            classes.filter(cls => ctrl.target === cls).length > 0);
    }
     ...
     */
    public filterInterceptorUsesWithTargetAndMethod(target: Function, methodName: string): Array<UseInterceptorMetadataArgs> {
        return this.useInterceptors.filter(use =>
            use.target === target && use.method === methodName);
    }
    ...
    public filterParamsWithTargetAndMethod(target: Function, methodName: string): Array<ParamMetadataArgs> {
        return this.params.filter(param =>
            param.object.constructor === target && param.method === methodName);
    }
     ...
     */
    public filterResponseHandlersWithTargetAndMethod(target: Function, methodName: string): Array<ResponseHandlerMetadataArgs> {
        return this.responseHandlers.filter(property =>
            property.target === target && property.method === methodName);
    }
