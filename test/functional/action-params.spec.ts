...
describe("action parameters", () => {
  ...
  let queryParams1: { [key: string]: any },
    queryParams2: { [key: string]: any },
    queryParams3: { [key: string]: any };
  ...
  before(() => {
    ...
    const {
      SetStateMiddleware,
    } = require("../fakes/global-options/koa-middlewares/SetStateMiddleware");
    const {
      SessionMiddleware,
    } = require("../fakes/global-options/SessionMiddleware");
    ...
    class UserActionParamsController {
      ...
      getUsersDirectKoa(@Ctx() ctx: any): any {
        ...
        ctx.response.body =
          "hi, I was written directly to the response using Koa Ctx";
        return ctx;
      }
      ...
      @UseBefore(SessionMiddleware)
      loadEmptyParamFromSession(
        @SessionParam("empty", { required: false }) emptyElement: string
      ) {
      ...
      @UseBefore(SessionMiddleware)
      errorOnLoadEmptyParamFromSession(
        @SessionParam("empty") emptyElement: string
      ) {
        sessionTestElement = emptyElement;
        return `<html><body>${emptyElement === undefined}</body></html>`;
      }
      ...
      getPhotosWithQueryAndNoValidation(
        @QueryParams({ validate: false }) query: QueryClass
      ) {
        queryParams2 = query;
        return `<html><body>hello</body></html>`;
      }
      ...
      getPhotosWithOptionalQuery(
        @QueryParams({ validate: { skipMissingProperties: true } })
        query: QueryClass
      ) {
        queryParams3 = query;
        return `<html><body>hello</body></html>`;
      ...
      getPostsWithIdRequired(
        @HeaderParam("limit", { required: true }) limit: number
      ) {
      ...
      getQuestionsWithIdRequired(
        @CookieParam("limit", { required: true }) limit: number
      ) {
      ...
      postFileWithBody(
        @UploadedFile("myfile") file: any,
        @Body() body: any
      ): any {
        uploadedFileName = file.originalname;
        return `<html><body>${uploadedFileName} - ${JSON.stringify(
          body
        )}</body></html>`;
      }
      ...
      postFileWithLimit(
        @UploadedFile("myfile", { options: { limits: { fileSize: 2 } } })
        file: any
      ): any {
      ...
      postFileWithRequired(
        @UploadedFile("myfile", { required: true }) file: any
      ): any {
        return `<html><body>${file.originalname}</body></html>`;
      }
      ...
      postPhotosWithLimit(
        @UploadedFiles("photos", { options: { limits: { files: 1 } } })
        files: any
      ): any {
      ...
      postPhotosWithRequired(
        @UploadedFiles("photos", { required: true }) files: any
      ): any {
        return `<html><body>${files[0].originalname}</body></html>`;
      }
    }
  ...
  describe("writing directly to the response using @Res should work", () => {
    assertRequest([3001, 3002], "get", "users-direct", (response) => {
      ...
      expect(response.body).to.be.equal(
        "hi, I was written directly to the response"
      );
      expect(response).to.have.header(
        "content-type",
        "custom/x-sample; charset=utf-8"
      );
    });
  });
  ...
  describe("@Session should return a value from session", () => {
    assertRequest([3001, 3002], "post", "session", (response) => {
      ...
      assertRequest([3001, 3002], "get", "session", (response) => {
        ...
        expect(response.body).to.be.equal(
          "<html><body>@Session test</body></html>"
        );
  ...
  describe("@QueryParams should give a proper values from request's query parameters", () => {
    assertRequest(
      [3001 /*3002*/],
      "get",
      "photos-params?sortBy=name&count=2&limit=10&showAll",
      (response) => {
        expect(response).to.be.status(200);
        expect(response).to.have.header(
          "content-type",
  ...
  describe("@QueryParams should give a proper values from request's optional query parameters", () => {
    assertRequest(
      [3001 /*3002*/],
      "get",
      "photos-params-optional?sortBy=name&limit=10",
      (response) => {
  ...
  describe("@QueryParam should give a proper values from request query parameters", () => {
    assertRequest(
      [3001, 3002],
      "get",
      "photos?sortBy=name&count=2&limit=10&showAll=true",
      (response) => {
  ...
  describe("for @QueryParam when the type is Date then it should be parsed", () => {
    assertRequest(
      [3001, 3002],
      "get",
      "posts-after/?from=2017-01-01T00:00:00Z",
      (response) => {
        expect(response).to.be.status(200);
  ...
  describe("for @QueryParam when parseJson flag is used query param must be converted to object", () => {
    assertRequest(
      [3001, 3002],
      "get",
      'photos-with-json/?filter={"keyword": "name", "limit": 5}',
      (response) => {
  ...
  describe("for @CookieParam when parseJson flag is used query param must be converted to object", () => {
    ...
    jar.setCookie(
      request.cookie('filter={"keyword": "name", "limit": 5}'),
      url
    );
  ...
  describe("@BodyParam should provide a json object for json-typed controllers and actions", () => {
    assertRequest(
      ...
      "post",
      "users",
      { name: "johny", age: 27, isActive: true },
      (response) => {
  ...
  describe("@BodyParam should fail if required body was not provided for json-typed controllers and actions", () => {
    ...
    assertRequest(
      ...
      "post",
      "users-with-required",
      { name: "Johny", age: 0, isActive: false },
      (response) => {
        expect(response).to.be.status(204);
      }
    );
    ...
    assertRequest(
      ...
      "post",
      "users-with-required",
      { name: "Johny", age: 27, isActive: false },
      (response) => {
        expect(response).to.be.status(204);
      }
    );
  ...
  describe("@UploadedFile should provide uploaded file with the given name", () => {
    ...
    assertRequest(
      ...
      (response) => {
        ...
        expect(response.body).to.be.equal(
          "<html><body>hello-world.txt</body></html>"
        );
      }
    );
  });
  ...
  describe("@UploadedFile with @BodyParam should return both the file and the body param", () => {
    ...
    assertRequest(
      ...
      (response) => {
        ...
        expect(response.body).to.be.equal(
          "<html><body>hello-world.txt - hi, i'm a param</body></html>"
        );
      }
    );
  });
  ...
  describe("@UploadedFiles should provide uploaded files with the given name", () => {
    ...
    assertRequest(
      ...
      (response) => {
        ...
        expect(response.body).to.be.equal(
          "<html><body>me.jpg she.jpg</body></html>"
        );
      }
    );
  });
