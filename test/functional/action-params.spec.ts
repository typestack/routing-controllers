...
beforeAll((done) => {
    ...
    class UserActionParamsController {
        ...
        getUserPhoto(@Param("firstId") firstId: number,
                     @Param("secondId") secondId: number): string {
        ...
        getPhotos(@QueryParam("sortBy") sortBy: string,
                  ...
                  @QueryParam("showAll") showAll: boolean): string {
        ...
        getPosts(@HeaderParam("token") token: string,
                 ...
                 @HeaderParam("showAll") showAll: boolean): string {
        ...
        getQuestions(@CookieParam("token") token: string,
                     ...
                     @CookieParam("showAll") showAll: boolean): string {
        ...
        getQuestionsWithJsonParam(@CookieParam("filter", {parse: true}) filter: { keyword: string, limit: number }): string {
        ...
        postFileWithBodyParam(@UploadedFile("myFile") file: Express.Multer.File, @BodyParam("testParam") testParam: string): string {
            return `<html><body>${file.originalname} - ${testParam}</body></html>`;
        }

        @Post("/file-with-limit")
        postFileWithLimit(@UploadedFile("myFile", {options: {limits: {fileSize: 2}}}) file: Express.Multer.File): string {
            return `<html><body>${file.originalname}</body></html>`;
        }
...
it("@Param multiple params should give a proper values from route", () => {
    expect.assertions(5);
    return axios.get("/users/23/photos/32")
        .then((response: AxiosResponse) => {
            expect(response.status).toEqual(HttpStatusCodes.OK);
            expect(response.headers["content-type"]).toEqual("text/html; charset=utf-8");
...
it("@SessionParam without middleware", () => {
    expect.assertions(1);
    return axios.get("/not-use-session")
...
it("@Session(param) should allow to inject empty property", () => {
    return axios.get("/session-param-empty")
        .then((response: AxiosResponse) => {
            expect(response.status).toEqual(HttpStatusCodes.OK);
            expect(response.headers["content-type"]).toEqual("text/html; charset=utf-8");
...
it("@QueryParams should give a proper values from request's query parameters", () => {
    expect.assertions(6);
    return axios.get("/photos-params?sortBy=name&count=2&limit=10&showAll")
        .then((response: AxiosResponse) => {
            expect(response.status).toEqual(HttpStatusCodes.OK);
            expect(response.headers["content-type"]).toEqual("text/html; charset=utf-8");
...
it("@QueryParams should give a proper values from request's query parameters with nested json", () => {
    expect.assertions(9);
    return axios.get("/photos-params?sortBy=name&count=2&limit=10&showAll&myObject=%7B%22num%22%3A%205,%20%22str%22%3A%20%22five%22,%20%22isFive%22%3A%20true%7D")
        .then((response: AxiosResponse) => {
            expect(response.status).toEqual(HttpStatusCodes.OK);
            expect(response.headers["content-type"]).toEqual("text/html; charset=utf-8");
            expect(queryParams1.sortBy).toEqual("name");
...
it("@QueryParams should not validate request query parameters when it's turned off in validator options", () => {
    expect.assertions(6);
    return axios.get("/photos-params-no-validate?sortBy=verylongtext&count=2&limit=1&showAll=true")
        .then((response: AxiosResponse) => {
            expect(response.status).toEqual(HttpStatusCodes.OK);
            expect(response.headers["content-type"]).toEqual("text/html; charset=utf-8");
...
it("@QueryParams should give a proper values from request's optional query parameters", () => {
    expect.assertions(6);
    return axios.get("/photos-params-optional?sortBy=name&limit=10")
        .then((response: AxiosResponse) => {
            expect(response.status).toEqual(HttpStatusCodes.OK);
            expect(response.headers["content-type"]).toEqual("text/html; charset=utf-8");
...
it("@QueryParam should give a proper values from request query parameters", () => {
    expect.assertions(6);
    return axios.get("/photos?sortBy=name&count=2&limit=10&showAll=true")
        .then((response: AxiosResponse) => {
            expect(response.status).toEqual(HttpStatusCodes.OK);
            expect(response.headers["content-type"]).toEqual("text/html; charset=utf-8");
...
it("@QueryParam when required params must be provided and they should not be empty", () => {
    ...
    return Promise.all<AxiosResponse | void>([
        axios.get("/photos-with-required?limit=0")
            .then((response: AxiosResponse) => {
        ...
        axios.get("/photos-with-required?")
            .catch((error: AxiosError) => {
                expect(error.response.status).toEqual(HttpStatusCodes.BAD_REQUEST);
            }),
        axios.get("/photos-with-required?limit")
            .catch((error: AxiosError) => {
                expect(error.response.status).toEqual(HttpStatusCodes.BAD_REQUEST);
            })
    ]);
...
it("@QueryParam when the type is Date then it should be parsed", () => {
    expect.assertions(2);
    return axios.get("/posts-after/?from=2017-01-01T00:00:00Z")
        .then((response: AxiosResponse) => {
            expect(response.status).toEqual(HttpStatusCodes.OK);
...
it("@QueryParam when the type is Date and it is invalid then the response should be a BadRequest error", () => {
    expect.assertions(2);
    return axios.get("/posts-after/?from=InvalidDate")
        .catch((error: AxiosError) => {
            expect(error.response.status).toEqual(HttpStatusCodes.BAD_REQUEST);
...
it("@QueryParam when parseJson flag is used query param must be converted to object", () => {
    expect.assertions(3);
    return axios.get("/photos-with-json/?filter={\"keyword\": \"name\", \"limit\": 5}")
        .then((response: AxiosResponse) => {
            expect(response.status).toEqual(HttpStatusCodes.OK);
            expect(response.headers["content-type"]).toEqual("text/html; charset=utf-8");
...
it("@HeaderParam when required is params must be provided and they should not be empty", () => {
    expect.assertions(3);
    return axios.get("/posts-with-required", {
        headers: {
            limit: 0
        }
    }).then((response: AxiosResponse) => {
...
it("for @HeaderParam when parseJson flag is used query param must be converted to object", () => {
    expect.assertions(3);
    return axios.get("/posts-with-json", {
        headers: {
...
it("@CookieParam when required is params must be provided and they should not be empty", () => {
    expect.assertions(4);
    return Promise.all<AxiosResponse | void>([
        axios.get("/questions-with-required", {
            headers: {
                Cookie: "limit=20"
            },
            withCredentials: true
...
it("@CookieParam when parseJson flag is used query param must be converted to object", () => {
    expect.assertions(3);
    return axios.get("/questions-with-json", {
        headers: {
...
it("@Body should provide a request body", () => {
    expect.assertions(3);
    return axios.post("/questions", "hello", {
        headers: {
...
it("@Body should fail if required body was not provided", () => {
    expect.assertions(5);
    return Promise.all<AxiosResponse | void>([
        axios.post("/questions-with-required", "0", {
            headers: {
                "Content-type": "text/plain"
            }
        }).then((response: AxiosResponse) => {
        ...
        axios.post("/questions-with-required", "", {
            headers: {
                "Content-type": "text/plain"
            }
        }).catch((error: AxiosError) => {
...
it("@Body should provide a json object for json-typed controllers and actions", () => {
    expect.assertions(4);
    return axios.post("/posts", {hello: "world"})
        .then((response: AxiosResponse) => {
...
it("@Body should fail if required body was not provided for json-typed controllers and actions", () => {
    expect.assertions(2);
    return Promise.all<AxiosResponse | void>([
        axios.post("posts-with-required", {hello: ""})
            .then((response: AxiosResponse) => {
                expect(response.status).toEqual(HttpStatusCodes.OK);
            }),
        axios.post("posts-with-required")
            .catch((error: AxiosError) => {
                expect(error.response.status).toEqual(HttpStatusCodes.BAD_REQUEST);
            })
    ]);
...
it("@BodyParam should provide a json object for json-typed controllers and actions", () => {
    expect.assertions(4);
    return axios.post("/users", {name: "johny", age: 27, isActive: true})
        .then((response: AxiosResponse) => {
...
it("@BodyParam should fail if required body was not provided for json-typed controllers and actions", () => {
    expect.assertions(8);
    return Promise.all<AxiosResponse | void>([
        axios.post("/users-with-required", {name: "johny", age: 27, isActive: true})
            .then((response: AxiosResponse) => {
                expect(response.status).toEqual(HttpStatusCodes.NO_CONTENT);
            }),
        axios.post("/users-with-required")
            .catch((error: AxiosError) => {
                expect(error.response.status).toEqual(HttpStatusCodes.BAD_REQUEST);
            }),
        axios.post("/users-with-required", {name: "", age: 27, isActive: false})
            .catch((error: AxiosError) => {
                expect(error.response.status).toEqual(HttpStatusCodes.BAD_REQUEST);
            }),
        axios.post("/users-with-required", {name: "Johny", age: 0, isActive: false})
            .then((response: AxiosResponse) => {
                expect(response.status).toEqual(HttpStatusCodes.NO_CONTENT);
            }),
        axios.post("/users-with-required", {name: "Johny", age: undefined, isActive: false})
            .catch((error: AxiosError) => {
                expect(error.response.status).toEqual(HttpStatusCodes.BAD_REQUEST);
            }),
        axios.post("/users-with-required", {name: "Johny", age: 27, isActive: undefined})
            .catch((error: AxiosError) => {
                expect(error.response.status).toEqual(HttpStatusCodes.BAD_REQUEST);
            }),
        axios.post("/users-with-required", {name: "Johny", age: 27, isActive: false})
            .then((response: AxiosResponse) => {
                expect(response.status).toEqual(HttpStatusCodes.NO_CONTENT);
            }),
        axios.post("/users-with-required", {name: "Johny", age: 27, isActive: true})
...
it("@Body using application/x-www-form-urlencoded should handle url encoded form data", () => {
    expect.assertions(3);
    return axios.post("/form-data-body", qs.stringify({
        testObject: {
            testNested: {
...
it("@UploadedFile using multipart/form-data should provide uploaded file with the given name", () => {
    ...
    return axios.post("/file", form, {
        ...
        expect(response.status).toEqual(HttpStatusCodes.OK);
        expect(response.headers["content-type"]).toEqual("text/html; charset=utf-8");
        expect(response.data).toEqual("<html><body>sample-text-file-one.txt</body></html>");
    });
});
...
it("@UploadedFile with @Body should return both the file and the body", () => {
    ...
    return axios.post("/file-with-body", form, {
        headers: form.getHeaders()
    }).then((response: AxiosResponse) => {
        expect(response.status).toEqual(HttpStatusCodes.OK);
        expect(response.headers["content-type"]).toEqual("text/html; charset=utf-8");
        expect(response.data).toEqual(`<html><body>sample-text-file-one.txt - {"anotherField":"hello","andAnother":"world"}</body></html>`);
    });
});
...
it("@UploadedFile with @BodyParam should return both the file and the body param", () => {
    ...
    return axios.post("/file-with-body-param", form, {
        ...
        expect(response.status).toEqual(HttpStatusCodes.OK);
        expect(response.headers["content-type"]).toEqual("text/html; charset=utf-8");
        expect(response.data).toEqual(`<html><body>sample-text-file-one.txt - testParamOne</body></html>`);
    });
});
...
it("@UploadedFile when required is used files must be provided", () => {
    ...
    return Promise.all<AxiosResponse | void>([
        ...
        axios.post("/file-with-required", undefined, {
            headers: form.getHeaders()
        }).catch((error: AxiosError) => {
            expect(error.response.status).toEqual(HttpStatusCodes.BAD_REQUEST);
        })
...
it("@UploadedFiles should provide uploaded files with the given name", () => {
    ...
    return axios.post("/photos", form, {
        ...
        expect(response.status).toEqual(HttpStatusCodes.OK);
        expect(response.headers["content-type"]).toEqual("text/html; charset=utf-8");
        expect(response.data).toEqual("<html><body>sample-text-file-one.txt sample-text-file-two.txt</body></html>");
    });
});
