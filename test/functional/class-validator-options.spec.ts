...
import { Length } from "@nest/class-validator";
...
import { defaultMetadataStorage } from "@nest/class-transformer/storage";
...
describe("parameters auto-validation", () => {
  ...
  describe("should apply global validation enable", () => {
    ...
    before(
      (done) => (expressApp = createExpressServer(options).listen(3001, done))
    );
    ...
    assertRequest(
      [3001, 3002],
      "get",
      'user?filter={"keyword": "Um", "__somethingPrivate": "blablabla"}',
      (response) => {
  ...
  describe("should apply local validation enable", () => {
    ...
    before(() => {
      ...
      class ClassTransformUserController {
        ...
        getUsers(
          @QueryParam("filter", { validate: true }) filter: UserFilter
        ): any {
          requestFilter = filter;
          const user = new UserModel();
          user.id = 1;
  ...
  describe("should contain param name on validation failed", () => {
    ...
    assertRequest(
      [3001, 3002],
      "get",
      `user?filter=${JSON.stringify(invalidFilter)}`,
      (response) => {
        expect(response).to.have.status(400);
