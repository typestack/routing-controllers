...
describe('controller > base routes functionality', () => {
  ...
  it('get should respond with proper status code, headers and body content', () => {
    expect.assertions(3);
    return axios.get('posts/1/users/2').then((response: AxiosResponse) => {
      expect(response.status).toEqual(HttpStatusCodes.OK);
      expect(response.headers['content-type']).toEqual('text/html; charset=utf-8');
  ...
  it('wrong route should respond with 404 error', () => {
    expect.assertions(1);
    return axios.get('/1/users/1').catch((error: AxiosError) => {
  ...
  it('wrong route should respond with 404 error', () => {
    expect.assertions(1);
    return axios.get('/categories/1').catch((error: AxiosError) => {
      expect(error.response.status).toEqual(HttpStatusCodes.NOT_FOUND);
    });
  });
  ...
  it('wrong route should respond with 404 error', () => {
    expect.assertions(1);
    return axios.get('/users/1').catch((error: AxiosError) => {
      expect(error.response.status).toEqual(HttpStatusCodes.NOT_FOUND);
    });
  });
});
