...
describe('koa middlewares', () => {
  ...
  it('should handle errors in custom middlewares', () => {
    return chakram.get('http://127.0.0.1:3001/customMiddlewareWhichThrows').then((response: any) => {
      expect(response).to.have.status(406);
    });
  });

  it('should handle errors in custom async middlewares', () => {
    return chakram.get('http://127.0.0.1:3001/customAsyncMiddlewareWhichThrows').then((response: any) => {
      expect(response).to.have.status(406);
    });
  });
});
